...
def lambda_handler(event, context):
    ...
    image_size_px = request_payload_dict.get("image_size_px", 200)
    upn_data = request_payload_dict.get("upn_data")
    if upn_data is None:
        return _lambda_proxy_response(
            status_code=http.HTTPStatus.BAD_REQUEST,
            body=json.dumps({
                "errors": ["'upn_data' is a required field"],
            }))

    upnqr = UPNQR.from_dict(request_payload_dict.get("upn_data"))
    ...
    image = upnqr.make_qr_code(image_size_px)
